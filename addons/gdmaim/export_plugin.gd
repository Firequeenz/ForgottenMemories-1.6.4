...
func _export_begin(features: PackedStringArray, is_debug: bool, path: String, flags: int) -> void :
    ...
    for symbol in settings.excluded_symbols.split(","):
        var trimmed: String = symbol.strip_edges()
        if !trimmed.is_empty():
            _symbols.lock_symbol_name(trimmed)
...
func _obfuscate_resource(path: String, source_data: String) -> String:
    ...
    while _src_obfuscators.has(embedded_path):
        ...
        _src_obfuscators.erase(embedded_path)
        index += 1
