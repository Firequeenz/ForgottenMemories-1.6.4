extends Resource
class_name OnOpponentSummonWithRockInGrave
...
func check_summon(summoned_slot, _card_data, trap: TrapData) -> bool:
    ...
    if not has_empty_slot:
        print("Release from Stone: Sem slot vazio. Não ativa.")
        return false
    ...
    if not has_rock:
        print("Release from Stone: Sem monstro Rock no cemitério. Não ativa.")
        return false
