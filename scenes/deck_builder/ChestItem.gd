...
func setup(card_id: int, card_data: CardData, chest_count: int, deck_count: int, can_add: bool):
    ...
    deck_count_label.text = ": %d" % [deck_count]
    ...
    if chest_count == 0:
        ...
        remove_button.disabled = (deck_count == 0)
    elif deck_count >= max_copies_allowed:
        is_selectable = false
        deck_count_label.modulate = Color("ff2645ff")
        add_button.disabled = true
        remove_button.disabled = (deck_count == 0)
    elif not can_add:
        is_selectable = false
        chest_count_label.modulate = Color("ff2645ff")
        add_button.disabled = true
        remove_button.disabled = (deck_count == 0)
    else:
...
func _on_gui_input(event):
    if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
        if is_selectable:
            ...
            tween.tween_property(self, "modulate", Color.WHITE, 0.2).set_delay(0.1)
